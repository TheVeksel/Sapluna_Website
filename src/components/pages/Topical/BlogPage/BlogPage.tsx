...
import { useGetAllBlogPostsQuery, useGetAllPostsQuery, useGetPostBySlugQuery } from "../../../../api/wpApi";
  ...
  return (
    <section className="blogi-page">
      <div className="wrapper">
        ...
        <div className="blogi-page__textbox"><p>{acf.text}</p></div>
          ...
          {(filteredPosts) => (
            <>
                  ...
                  return (
                    <li key={post.id} className="blogi-item">
                      <Link to={`/ajankohtaista/blogi/${post.slug}`}>
                        {img ? (
                          <img src={img} alt={post.title.rendered} className="thumbnail" />
                        ) : (
                          <div className="thumbnail-placeholder">No image</div>
                        )}
                        {category && <span className="acf-category" style={{backgroundColor: categories[category]}}>{category}</span>}
              ...
              {filteredPosts.length === 0 && (
                <p className="no-results">Ei tuloksia valituilla suodattimilla.</p>
              )}
            </>
          )}
        </BlogFilters>
